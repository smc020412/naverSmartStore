...
shipping_fee_file = st.sidebar.file_uploader(
    "상품현황 배송비 + 가격 파일 업로드 (선택)",
    type=["xlsx"], key="shipping_fee"
)
if shipping_fee_file:
    try:
        ...
        st.sidebar.success(
            f"배송비 매핑 {len(shipping_map)}건, 판매가격 매핑 {len(price_map)}건 로드됨"
        )
...
def fill_price(row):
    orig = row['판매금액']
    if not isinstance(orig, (int, float)) or pd.isna(orig) or orig == 0:
        return price_map.get((row['판매품목'], row['옵션명']), 0)
    return orig
