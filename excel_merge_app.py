...
# 0) 상품목록 파일 업로드 및 배송비 매핑
item_file = st.sidebar.file_uploader(
    "아산율림_상품목록 파일 업로드", type=["xlsx"], accept_multiple_files=False, key="item_file"
)
if not item_file:
    st.sidebar.error("아산율림_상품목록.xlsx 파일을 업로드해주세요.")
    st.stop()
...
for df in (df_ok, df_err):
    df['택배비'] = df['판매품목'].map(shipping_map) * df['판매수량']
    df['택배비'] = df['택배비'].fillna(0)
...
with pd.ExcelWriter(buf, engine='openpyxl') as writer:
    def write_with_summary(df, sheet_name):
        ...
        ws.cell(row=summary_row+1, column=col_qty, value='총금액')
        ws.cell(row=summary_row+1, column=col_qty+1, value=total_amount)
        ws.cell(row=summary_row+2, column=col_qty+2, value='총수수료')
        ws.cell(row=summary_row+2, column=col_qty+3, value=total_fee)
        ws.cell(row=summary_row+3, column=col_qty+2, value='총택배비')
        ws.cell(row=summary_row+3, column=col_qty+3, value=total_delivery)
        ws.cell(row=summary_row+4, column=col_qty+2, value='총지출')
        ws.cell(row=summary_row+4, column=col_qty+3, value=total_deposit)
        ws.cell(row=summary_row+5, column=col_qty, value='총이익')
        ws.cell(row=summary_row+5, column=col_qty+1, value=total_amount + total_fee + total_delivery)
